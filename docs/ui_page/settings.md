...
**Related:** [LabelsDataTable](./settings_EntityListOfInstancesTable.md) • [EntityListOfInstancesTable](./entity_EntityListOfInstancesTable.md) • [Overview](./data_table.md)
...
| **Entity Validation** | Only entities from `d_entity` can have datalabels |
...
2. Select Entity: "Employee" (from d_entity)
...
|-----------|--------|---------|------------|
| **entity_code** | lowercase, underscores | `employee`, `task` | Must exist in `d_entity.code` |
...
- Validate entity exists in `d_entity` before creating datalabel
