...
### 3.4 Reactive Entity Field Pattern (5 Layers)
...
|-------|------|---------------|------------|---------|-------------|
| **globalSettings** | `useGlobalSettings()` | `getSettingSync()` | 10 min | 1 hour | Dexie (24h) |
| **datalabel** | `useDatalabel(key)` | `getDatalabelSync(key)` | 10 min | **Infinity** | Dexie (24h) |
| **entityCodes** | `useEntityCodes()` | `getEntityCodesSync()` | 30 min | **Infinity** | Dexie (24h) |
| **entityInstMetadata** | `useEntityInstanceMetadata()` | `getEntityInstanceMetadataSync()` | 1 min | 1 hour | Dexie (24h) |
| **entityInstNames** | `useEntityInstanceNames()` | `getEntityInstanceNameSync()` | 10 min | 1 hour | Dexie (24h) |
| **entityLinks** | `useEntityLinks()` | `getChildIdsSync()` | 5 min | 1 hour | Dexie (24h) |
| **entityInstData** | `useEntityInstanceData()` | - | 1 min | 30 min | Dexie (30m) |
| **draft** | `useDraft()` | - | Never | Forever | Dexie |

### 4.3 Complete Cache Store Architecture
...
│                                                                                                      │
│ ┌─────────────────────────────────────────────────────────────────────────────────────────────────┐ │
│ │ SESSION-LEVEL STORES (Hydrated at Login, 10-min stale)                                          │ │
│ ├─────────────────────────────────────────────────────────────────────────────────────────────────┤ │
│ │                                                                                                  │ │
│ │ ┌──────────────────────┐ ┌──────────────────────┐ ┌──────────────────────┐                      │ │
...
│ │ │ global               │ │ ?name=dl__*          │ │                      │                      │ │
│ │ │                      │ │                      │ │                      │                      │ │
│ │ │ Stale: 10 min        │ │ Stale: 10 min        │ │ Stale: 30 min        │                      │ │
│ │ │ GC: 1 hour           │ │ GC: Infinity         │ │ GC: Infinity         │                      │ │
│ │ └──────────────────────┘ └──────────────────────┘ └──────────────────────┘                      │ │
│ │                                                                                                  │ │
│ │ ┌──────────────────────┐ ┌──────────────────────┐                                               │ │
...
│                                                                                                      │
│ ┌─────────────────────────────────────────────────────────────────────────────────────────────────┐ │
│ │ ON-DEMAND STORES (Fetched when needed, 1-5 min stale)                                           │ │
│ ├─────────────────────────────────────────────────────────────────────────────────────────────────┤ │
│ │                                                                                                  │ │
│ │ ┌──────────────────────┐ ┌──────────────────────┐ ┌──────────────────────┐                      │ │
...
│ │ │   ?limit=N           │ │   {id}/children      │ │                      │                      │ │
│ │ │                      │ │                      │ │                      │                      │ │
│ │ │ Stale: 1 min         │ │ Stale: 5 min         │ │ Stale: Never         │                      │ │
│ │ │ GC: 30 min           │ │ GC: 1 hour           │ │ GC: Until saved      │                      │ │
│ │ └──────────────────────┘ └──────────────────────┘ └──────────────────────┘                      │ │
│ │                                                                                                  │ │
│ └─────────────────────────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                                      │
...
│  ├── entityInstData → Project records                                                              │
│  └── ref_data_entityInstance → Inline with data response (manager names)                           │
│                                                                                                      │
...
UI: <Table columns={columns}><LoadingIndicator /></Table>
...
### 7.4 EntityListOfInstancesPage Component Architecture
...
├── useEntityInstanceMetadata()      // TanStack Query hook (metadata, 1-min stale)
├── useEntityInstanceData()          // TanStack Query hook (data, 1-min stale)
├── useFormattedEntityData()         // Format-at-read transformation (reactive)
├── useOptimisticMutation()          // Create/Update/Delete with rollback
│
├── EntityListOfInstancesTable       // Table view (default)
│   ├── Column headers from metadata
│   ├── Pagination controls
│   ├── InlineEdit cell editing
...
│  ─────────────────────────────                                               │
│  For dropdown fields (BadgeDropdownSelect, EntityInstanceNameSelect):       │
│  • Single selection → onChange fires once → immediate save                  │
│  • No debounce needed (atomic action, not continuous typing)                │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
// DROPDOWN (BadgeDropdownSelect): Immediate save on selection
<BadgeDropdownSelect
  value={currentValue}
...
const handleCellSave = useCallback((rowId, columnKey, record, valueOverride?) => {
  ...
  if (valueToSave !== originalValue) {
    // Push to undo stack
    setUndoStack(prev => [...prev, { rowId, columnKey, oldValue, newValue: valueToSave, record }]);

    // Trigger optimistic update via parent callback
...
│  ↓                                                                           │
│  LAYER 3: Edit Component Resolution                                         │
│  ────────────────────────────────────                                       │
│  • isEditing = true                                                          │
│  • editType.inputType = 'component'                                         │
│  • editType.component = 'BadgeDropdownSelect'                               │
│  • renderEditModeFromMetadata() renders <BadgeDropdownSelect />             │
│  ↓                                                                           │
│  LAYER 4: Portal Dropdown Renders                                           │
│  ──────────────────────────────────                                         │
│  • BadgeDropdownSelect.setDropdownOpen(true)                                │
│  • createPortal(<div data-dropdown-portal ref={dropdownRef}>...</div>)      │
│  ↓                                                                           │
│  USER: Click dropdown option "In Progress"                                  │
│  ───────────────────────────────────────                                    │
│  ↓                                                                           │
│  LAYER 5: Portal-Aware Click-Outside (Defense in Depth)                     │
│  ─────────────────────────────────────────────────────                      │
...
│  • onCellSave callback → EntityListOfInstancesPage.handleCellSave():       │
│    - transformForApi({ dl__project_stage: 'in_progress' })                 │
│    - await updateEntity(rowId, transformedData)                             │
│  • useOptimisticMutation.updateEntity():                                   │
│    - onMutate: Update TanStack cache + Dexie immediately                   │
│    - mutationFn: PATCH /api/v1/project/{id}                                │
...
- Save on: blur, Enter, Tab
- Cancel on: Escape
- Undo: Cmd+Z / Ctrl+Z with toast notification
...
─────────────────────────────────────────────────────────────────────
• Click-outside detection MUST check [data-dropdown-portal] attribute
• Must NOT close dropdown when clicking dropdown options
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│ MUST WORK - Non-Negotiable Behaviors                                        │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
...
│     (Portal must not close on option click)                                 │
│                                                                              │
...
│     Click ✕ → temp row removed from cache                                   │
│                                                                              │
│  5. UNDO (Cmd+Z)                                                             │
...
│                                                                              │
│  7. ESCAPE TO CANCEL                                                         │
│     Edit cell → Escape → original value restored, edit cancelled           │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
