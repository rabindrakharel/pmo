...
| **`entityInstanceDataStore`** | Single entity detail | Complete project record | URL-bound + 5 min | Memory |
| **`entityInstanceListDataStore`** | Table/list data | 20 project records | URL-bound + 5 min | Memory |
| **`useEntityEditStore`** | Edit state, dirty tracking | `{dirtyFields: Set(['budget'])}` | No TTL | Memory |

### Session-Level Stores with Dedicated Endpoints (30 min TTL)

| Store | Dedicated Endpoint | Used By | Invalidates When |
|-------|-------------------|---------|------------------|
| `globalSettingsMetadataStore` | `GET /api/v1/settings/global` | App-wide formatting | Exit `/settings/*` OR 30 min timer |
| `datalabelMetadataStore` | `GET /api/v1/settings/datalabels/all` | All dropdown fields | Exit `/settings/*` OR 30 min timer |
| `entityCodeMetadataStore` | `GET /api/v1/entity/types` | Sidebar navigation | Exit `/settings/*` OR 30 min timer |

### Session-Level Store WITHOUT Dedicated Endpoint (30 min TTL)

| Store | Populated By | Used By | Invalidates When |
|-------|--------------|---------|------------------|
| `entityComponentMetadataStore` | Entity list/detail API responses | EntityDataTable, EntityFormContainer | When entity data refreshes OR 30 min timer |
...
| Store | Cache Key Format | Fetched When | Invalidation |
|-------|------------------|--------------|--------------|
| `entityInstanceListDataStore` | URL path: `/office?page=1&limit=20` | Navigate to `/office` | URL exit + 5 min TTL |
| `entityInstanceDataStore` | URL path: `/office/uuid-123` | Navigate to `/office/:id` | URL exit + 5 min TTL |
...
|------|-------------|------------------|-----------|
| 09:00 | Login | Store all 30-min caches (entity codes, settings, datalabels) | 3 calls (initial load) |
| 09:01 | Click "Office" in sidebar | • Use cached entity codes ✓<br>• Fetch office list → `entityInstanceListDataStore` (URL-bound)<br>• Use cached datalabels ✓ | 1 call (GET /office) |
| 09:02 | Click office row #123 | • INVALIDATE office list (left URL)<br>• Fetch office/123 → `entityInstanceDataStore` (URL-bound)<br>• Reuse component metadata ✓ | 1 call (GET /office/123) |
| 09:03 | Edit budget field | • Track in `useEntityEditStore`<br>• Mark isDirty in `entityInstanceDataStore` | 0 calls |
| 09:04 | Save changes | • PATCH only changed field<br>• Clear isDirty flag<br>• Refresh instance cache | 1 call (PATCH /office/123) |
| 09:05 | Back to /office | • INVALIDATE office/123 (left URL)<br>• Refetch list<br>• Reuse 30-min caches ✓ | 1 call (GET /office) |
| 09:10 | Stay on /office | • 5 min TTL expires → background refetch | 1 call (GET /office) |
| 09:30 | Exit /settings → /project | • INVALIDATE all 30-min caches<br>• Refetch everything | 4 calls (all caches) |
...
const OfficePage = () => {
  ...
  // Hook automatically:
  // 1. Checks entityInstanceListDataStore for cached data
  // 2. Fetches if expired (>5 min) or URL changed
  // 3. Stores in entityInstanceListDataStore with URL as key
...
const OfficeDetailPage = () => {
  ...
  // Hook automatically:
  // 1. Checks entityInstanceDataStore for cached data
  // 2. Fetches if expired (>5 min) or URL changed
  // 3. Stores in entityInstanceDataStore with 'entityCode:id' as key
...
useEffect(() => {
  const unsubscribe = navigation.subscribe((location) => {
    ...
    if (previousPath !== currentPath) {
      ...
      if (prevEntity) {
        entityInstanceListDataStore.invalidate(prevEntity);
        entityInstanceDataStore.invalidateEntity(prevEntity);
      }
    }

    // INVALIDATE session caches when leaving settings
    if (previousPath?.startsWith('/settings') && !currentPath.startsWith('/settings')) {
      globalSettingsMetadataStore.clear();
      datalabelMetadataStore.clear();
      entityCodeMetadataStore.clear();
      entityComponentMetadataStore.clear();
    }
...
| `useEntityInstanceList` | Navigate to `/entity` | `entityInstanceListDataStore` | `['entity-instance-list', entityCode, params]` | URL-bound + 5 min |
| `useEntityInstance` | Navigate to `/entity/:id` | `entityInstanceDataStore` | `['entity-instance', entityCode, id]` | URL-bound + 5 min |
...
5. **Optimistic Updates**: `isDirty` flag in `entityInstanceDataStore` enables immediate UI feedback
...
SHORT-LIVED (5 min TTL + URL-bound, memory):
├── entityInstanceDataStore         → Single entity (/entity/:id)
└── entityInstanceListDataStore     → List data (/entity?params)
