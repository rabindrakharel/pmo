...
--
-- Purpose:
--   Geographic location hierarchy dimension representing the complete
--   Canadian geographic structure from Corp-Region (level 0) through
--   Address (level 7). Provides foundation for location-based operations,
...
--
-- Integration:
--   - References meta_location_level table for level definitions
--   - Supports geographic reporting and jurisdictional compliance
--   - Enables location-based service delivery and resource allocation
...
CREATE TABLE app.d_scope_location (
  ...
  location_code text,
  level_id int NOT NULL REFERENCES app.meta_location_level(level_id),
...
INSERT INTO app.d_scope_location (name, "descr", location_code, level_id, level_name, parent_id, is_leaf_level, country_code, time_zone, service_area, regulatory_jurisdiction, tags, attr) VALUES
...
WITH corp_region AS (SELECT id FROM app.d_scope_location WHERE name = 'North America Central')
INSERT INTO app.d_scope_location (name, "descr", location_code, level_id, level_name, parent_id, is_leaf_level, country_code, time_zone, service_area, regulatory_jurisdiction, tags, attr)
SELECT 
  'Canada',
...
WITH canada AS (SELECT id FROM app.d_scope_location WHERE name = 'Canada')
INSERT INTO app.d_scope_location (name, "descr", location_code, level_id, level_name, parent_id, is_leaf_level, country_code, province_code, time_zone, service_area, regulatory_jurisdiction, tags, attr)
SELECT 
  'Ontario',
...
WITH ontario AS (SELECT id FROM app.d_scope_location WHERE name = 'Ontario')
INSERT INTO app.d_scope_location (name, "descr", location_code, level_id, level_name, parent_id, is_leaf_level, country_code, province_code, time_zone, service_area, regulatory_jurisdiction, stats_can_code, tags, attr)
SELECT 
...
WITH gta AS (SELECT id FROM app.d_scope_location WHERE name = 'Greater Toronto Area')
INSERT INTO app.d_scope_location (name, "descr", location_code, level_id, level_name, parent_id, is_leaf_level, country_code, province_code, time_zone, service_area, regulatory_jurisdiction, stats_can_code, tags, attr)
SELECT 
  'Toronto CMA',
...
-- City Level (Level 5)
WITH toronto_cma AS (SELECT id FROM app.d_scope_location WHERE name = 'Toronto CMA')
INSERT INTO app.d_scope_location (name, "descr", location_code, level_id, level_name, parent_id, is_leaf_level, country_code, province_code, postal_code, time_zone, service_area, regulatory_jurisdiction, municipal_code, latitude, longitude, tags, attr)
SELECT 
  'Mississauga',
  'City of Mississauga - corporate headquarters location and primary operational hub with diverse residential and commercial client base',
  'MISS',
  5,
  'City',
  toronto_cma.id,
  ...
  '{"population": 721599, "area_km2": 292.4, "incorporation": 1974, "headquarters": true, "business_friendly": true, "diverse_economy": true, "major_employers": ["banking", "technology", "logistics"], "strategic_location": true}'::jsonb
FROM toronto_cma
...
WITH mississauga AS (SELECT id FROM app.d_scope_location WHERE name = 'Mississauga')
INSERT INTO app.d_scope_location (name, "descr", location_code, level_id, level_name, parent_id, is_leaf_level, country_code, province_code, postal_code, time_zone, service_area, regulatory_jurisdiction, latitude, longitude, tags, attr)
SELECT 
  'City Centre',
...
WITH city_centre AS (SELECT id FROM app.d_scope_location WHERE name = 'City Centre')
INSERT INTO app.d_scope_location (name, "descr", location_code, level_id, level_name, parent_id, is_leaf_level, country_code, province_code, postal_code, time_zone, service_area, regulatory_jurisdiction, latitude, longitude, tags, attr)
SELECT 
...
SELECT 
  ...
  '{"commercial_hub": true, "high_density": true, "transit_accessible": true, "mixed_clientele": true, "premium_location": true}'::jsonb
FROM city_centre;
