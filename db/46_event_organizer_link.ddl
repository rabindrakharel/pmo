-- =====================================================
-- EVENT (d_event) - Add organizer__employee_id column
-- Universal parent entity for all event-related linkages
-- =====================================================
--
-- MIGRATION: Add organizer__employee_id, venue_type, and event_action_entity columns to existing d_event table
--

-- Add organizer__employee_id column to track which employee organized the event
ALTER TABLE app.event
ADD COLUMN IF NOT EXISTS organizer__employee_id uuid;
...
-- Update existing events to set organizer__employee_id from RBAC Owner permission
UPDATE app.event e
SET organizer__employee_id = r.person_id
FROM app.entity_rbac r
WHERE r.entity_code = 'event'
  AND r.entity_instance_id = e.id
  AND r.permission = 7
  AND r.person_code = 'employee'
  AND e.organizer__employee_id IS NULL;

-- Add comments for new columns
COMMENT ON COLUMN app.event.organizer__employee_id IS 'UUID of the employee who organized the event';
...
-- FROM app.event e
-- LEFT JOIN app.app.employee emp ON e.organizer__employee_id = emp.id
