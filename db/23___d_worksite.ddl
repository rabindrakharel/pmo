...
--
-- Integration:
--   - References d_org for geographic placement
...
CREATE TABLE app.d_worksite (
  ...
  org_id uuid REFERENCES app.d_org(id),
  addr text,
  postal_code text,
  ...
  -- Contact information
  site_manager_id uuid REFERENCES app.d_employee(id),
...
WITH toronto AS (SELECT id FROM app.d_org WHERE name = 'Toronto')
INSERT INTO app.d_worksite (
  name, "descr", worksite_code, worksite_type, org_id, addr, postal_code,
  latitude, longitude, capacity_workers, equipment_storage, vehicle_parking,
  security_required, indoor_space_sqft, outdoor_space_sqft, office_space,
...
('Toronto Service Center',
 ...
 'HHS-TOR-SC',
 'branch',
 (SELECT id FROM app.d_org WHERE name = 'Toronto'),
...
INSERT INTO app.d_worksite (
  ...
  security_required, indoor_space_sqft, outdoor_space_sqft, office_space,
  washroom_facilities, power_available, water_available, safety_rating,
  safety_last_inspection, seasonal_use, seasonal_period, tags, attr
) VALUES
('Winter Ops - Equipment Staging',
 ...
 'HHS-WINTER-STG',
 'seasonal',
 (SELECT id FROM app.d_org WHERE name = 'Hamilton CMA'),
...
('Solar Install - 1847 Sheridan Park Dr',
 ...
 'HHS-SOL-1847',
 'project',
 (SELECT id FROM app.d_org WHERE name = 'Oakville'),
...
('Landscaping - Square One Plaza',
 ...
 'HHS-LAND-SQ1',
 'project',
 (SELECT id FROM app.d_org WHERE name = 'City Centre'),
...
('Equipment Storage - Meadowvale',
 ...
 'HHS-EQUIP-MV',
 'storage',
 (SELECT id FROM app.d_org WHERE name = 'Meadowvale'),
