...
-- • code: varchar(50) UNIQUE NOT NULL (e.g., 'HS_STD', 'HVAC_EMERG', 'CONST_HOME')
-- • name: text NOT NULL (display name: 'Home Services - Standard Project')
-- • industry_sector: text NOT NULL (links to setting_datalabel industry_sector)
-- • workflow_graph: jsonb NOT NULL (array of state nodes with transitions)
-- • active_workflow_flag: boolean DEFAULT true (can new instances use this template?)
-- • default_workflow_flag: boolean DEFAULT false (default workflow for industry?)
-- • avg_duration_days: integer (historical average completion time)
-- • success_rate_pct: decimal(5,2) (historical success rate)
--
