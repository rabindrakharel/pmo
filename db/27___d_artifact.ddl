...
-- Design Artifacts
-- Note: Employee references will be updated after employee data is loaded
...
('Landscape Design Template - Residential Premium', 'Standardized landscape design template for premium residential clients including plant selection, hardscape elements, and seasonal considerations created by James Miller for Huron Home Services operations', 'DESIGN-LAND-RES-001', 'design', 'template', '2.1', 'file', '/artifacts/designs/landscape_residential_premium_v2.1.pdf', 'confidential', 'approved', '2024-03-15', 'en',
(SELECT id FROM app.d_employee WHERE email = 'james.miller@huronhome.ca'),
(SELECT id FROM app.d_employee WHERE email = 'james.miller@huronhome.ca'),
'["design", "landscape", "residential", "template"]',
'{"design_type": "landscape_template", "client_segment": "premium_residential", "includes": ["plant_selection", "hardscape", "seasonal_planning"], "software": "AutoCAD", "format": "PDF", "created_by": "James Miller", "business_impact": "high"}'),

('Commercial Plaza Design Standards', 'Design standards and guidelines for commercial plaza landscaping projects including accessibility compliance and maintenance considerations developed under James Miller leadership', 'DESIGN-LAND-COM-001', 'design', 'standards', '1.3', 'file', '/artifacts/designs/commercial_plaza_standards_v1.3.pdf', 'internal', 'approved', '2024-01-20', 'en',
(SELECT id FROM app.d_employee WHERE email = 'james.miller@huronhome.ca'),
(SELECT id FROM app.d_employee WHERE email = 'james.miller@huronhome.ca'),
'["design", "standards", "commercial", "plaza"]',
'{"design_type": "standards_document", "compliance": ["accessibility", "municipal"], "maintenance_focus": true, "plaza_types": ["retail", "office", "mixed_use"], "approved_by": "James Miller"}'),
