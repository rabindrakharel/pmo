...
-- Permissions resolve via UNION of role permissions (via d_entity_instance_link) and direct employee permissions.
...
--   1. Role-based permissions: employee → roles (via d_entity_instance_link) → permissions
...
CREATE TABLE app.d_entity_rbac (
  ...
  -- Entity target
  entity_code varchar(50) NOT NULL, -- Entity code (references d_entity.code): project, task, employee, office, business, worksite, customer, etc.
...
CREATE INDEX idx_rbac_person_entity ON app.d_entity_rbac(person_entity_name, person_entity_id, entity_code, entity_id) WHERE active_flag = true;

-- Index for permission resolution via roles
CREATE INDEX idx_rbac_role_entity ON app.d_entity_rbac(person_entity_name, person_entity_id, entity_code) WHERE person_entity_name = 'role' AND active_flag = true;

-- Index for expiration cleanup
CREATE INDEX idx_rbac_expires ON app.d_entity_rbac(expires_ts) WHERE expires_ts IS NOT NULL AND active_flag = true;

COMMENT ON TABLE app.d_entity_rbac IS 'Person-based RBAC system with integer permission levels: 0=View, 1=Edit, 2=Share, 3=Delete, 4=Create, 5=Owner. Higher levels automatically inherit all lower permissions via >= comparison. Supports both role-based (via d_entity_instance_link) and direct employee permissions. Permissions resolve via UNION, taking MAX level.';
COMMENT ON COLUMN app.d_entity_rbac.person_entity_name IS 'Type of person: employee (direct permission) or role (inherited by all employees assigned to that role via d_entity_instance_link)';
COMMENT ON COLUMN app.d_entity_rbac.person_entity_id IS 'UUID of employee (if person_entity_name=employee) or role (if person_entity_name=role)';
COMMENT ON COLUMN app.d_entity_rbac.entity_code IS 'Target entity code (references d_entity.code): project, task, employee, office, business, worksite, customer, service, product, order, invoice, etc.';
COMMENT ON COLUMN app.d_entity_rbac.entity_id IS 'Target entity UUID for instance-level permissions, or "11111111-1111-1111-1111-111111111111" for type-level permissions granting access to ALL instances';
COMMENT ON COLUMN app.d_entity_rbac.permission IS 'Permission level with automatic inheritance: 0=View, 1=Edit (implies View), 2=Share (implies Edit+View), 3=Delete (implies Share+Edit+View), 4=Create (implies all lower), 5=Owner (implies all permissions). Check using: permission >= required_level';
COMMENT ON COLUMN app.d_entity_rbac.granted_by_employee_id IS 'Employee who granted this permission - enables delegation tracking and audit trail';
COMMENT ON COLUMN app.d_entity_rbac.expires_ts IS 'Optional expiration timestamp for temporary permissions (contractor access, time-limited delegation)';
