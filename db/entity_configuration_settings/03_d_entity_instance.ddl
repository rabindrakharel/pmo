...
--    • Database: UPDATE entity_instance SET entity_name=$1, entity_code=$2, updated_ts=now() WHERE entity_code=$3 AND entity_id=$4
--    • SCD Behavior: IN-PLACE UPDATE
--      - Synchronizes name and code changes from source entity
--      - Maintains referential consistency across entity_instance_link and d_entity_rbac
...
--    • Database: UPDATE entity_instance SET active_flag=false, updated_ts=now() WHERE entity_code=$1 AND entity_id=$2
...
--    • Database: SELECT EXISTS(SELECT 1 FROM entity_instance WHERE entity_code=$1 AND entity_id=$2 AND active_flag=true)
...
--
-- RELATIONSHIPS:
-- • (entity_type, entity_id) ← entity_instance_link (parent-child relationships)
...
CREATE TABLE app.entity_instance (
    entity_code varchar(50) NOT NULL, -- References entity.code (no FK for loose coupling)
...
COMMENT ON COLUMN app.entity_instance.entity_code IS 'Entity type code from entity.code (project, task, employee, etc.)';
COMMENT ON COLUMN app.entity_instance.entity_id IS 'Unique identifier (UUID) of the specific entity instance';
COMMENT ON COLUMN app.entity_instance.entity_name IS 'Name of the entity instance (cached from source table)';
COMMENT ON COLUMN app.entity_instance.code IS 'Instance code (e.g., PROJ-001, EMP-123) cached from source table';
...
INSERT INTO app.entity_instance (entity_code, entity_id, entity_name, code)
