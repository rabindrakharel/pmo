...
- `clientApi`, `orgApi`, `worksiteApi`, `roleApi`
...
● Added a comprehensive 🔐 Complete Authentication & Data Flow Architecture section that explains:
  ...
  display, with RBAC security enforced at every step.
    ...
    1. Authentication: Uses preHandler: [fastify.authenticate] or checks request.user?.sub
  2. Authorization: Uses hasPermissionOnEntityId(userId, entityType, entityId, action) function
  3. List endpoints: Use getEmployeeEntityIds(userId, entityType) to filter accessible entities
  4. Individual endpoints: Use hasPermissionOnEntityId for specific permission checks


  1. JWT Login System: There's a /api/v1/auth/login endpoint that takes email/password and returns a JWT token
  2. JWT Verification: The fastify.authenticate middleware verifies JWT tokens and sets request.user
  3. RBAC System: Uses hasPermissionOnEntityId for individual permissions and getEmployeeEntityIds for filtering lists
