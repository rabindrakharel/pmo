    ...
    try {
        ...
        sql`(
          EXISTS (
            ...
            WHERE rbac.empid = ${userId}::uuid
              AND rbac.entity = 'business_hierarchy'
              AND (rbac.entity_id = bh.id::text OR rbac.entity_id = 'all')
      ...
      const nodes = await db.execute(sql`
        SELECT
          ...
          bh.manager_employee_id, bh.budget_allocated_amt,
          bh.from_ts, bh.to_ts, bh.active_flag, bh.created_ts, bh.updated_ts, bh.version,
          emp.name as manager_name,
          parent.name as parent_name
        FROM app.d_business_hierarchy bh
        LEFT JOIN app.d_employee emp ON bh.manager_employee_id = emp.id
