    ...
    try {
      ...
      if (eventIds.length > 0) {
        const attendeesQuery = client`
          ...
          LEFT JOIN app.customer cust ON epc.person_id = cust.id AND epc.person_entity_type = 'customer'
  ...
  fastify.get<{
    ...
    try {
      ...
      const attendeesQuery = client`
        ...
        LEFT JOIN app.customer cust ON epc.person_entity_type = 'customer' AND epc.person_id = cust.id
