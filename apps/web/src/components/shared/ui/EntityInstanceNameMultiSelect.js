 ...
 */
export function EntityInstanceNameMultiSelect({ entityCode, value = [], onChange, disabled = false, required = false, placeholder, className = '', maxDisplayed = 5 }) {
    ...
    const selectedOptions = localValue
        ...
        if (cachedName)
            return { value: uuid, label: cachedName };
    ...
    return (React.createElement("div", { ref: containerRef, className: `relative w-full ${className}` },
        ...
        ` },
            selectedOptions.length === 0 ? (React.createElement("span", { className: "text-gray-500 text-sm py-0.5" }, placeholder || `Select ${entityCode}...`)) : (React.createElement(React.Fragment, null,
                displayedChips.map(option => (React.createElement(Chip, { key: option.value, label: option.label, size: "sm", maxWidth: "120px", colorClass: "bg-slate-100 text-slate-700 border border-slate-300", removable: !disabled, onRemove: () => removeItem(option.value) }))),
                overflowCount > 0 && (React.createElement("span", { className: "text-xs text-slate-500 font-medium px-1" },
                    "+",
                    overflowCount,
                    " more")))),
            React.createElement("div", { className: "ml-auto flex items-center gap-1" },
                isLoading && React.createElement(InlineSpinner, { size: "sm" }),
                React.createElement(ChevronDown, { className: `w-4 h-4 text-gray-400 transition-transform ${isOpen ? 'rotate-180' : ''}` }))),
        isOpen && !disabled && (React.createElement("div", { className: "absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-72 overflow-hidden" },
            React.createElement("div", { className: "p-2 border-b border-gray-200 bg-gray-50" },
                React.createElement("div", { className: "relative" },
                    React.createElement(Search, { className: "absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" }),
                    React.createElement("input", { ref: searchInputRef, type: "text", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), placeholder: `Search ${entityCode}...`, className: "w-full pl-8 pr-3 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-slate-500 focus:border-slate-500" }))),
            React.createElement("div", { className: "max-h-56 overflow-y-auto" }, isLoading ? (React.createElement("div", { className: "px-3 py-4 text-center" },
                React.createElement(InlineSpinner, { size: "md" }),
                React.createElement("span", { className: "text-sm text-gray-500 ml-2" }, "Loading..."))) : filteredOptions.length === 0 ? (React.createElement("div", { className: "px-3 py-3 text-sm text-gray-500 text-center" }, searchTerm ? 'No matches found' : `No ${entityCode} available`)) : (filteredOptions.map(option => {
                const isSelected = localValue.includes(option.value);
                return (React.createElement("div", { key: option.value, onClick: () => toggleOption(option.value), className: `
                    ...
                    ` }, isSelected && React.createElement(Check, { className: "w-3 h-3 text-white" })),
                    React.createElement("span", { className: `text-sm truncate ${isSelected ? 'text-slate-700 font-medium' : 'text-gray-700'}` }, option.label)));
            }))),
            selectedOptions.length > 0 && (React.createElement("div", { className: "px-3 py-2 border-t border-gray-200 bg-gray-50 text-xs text-gray-500" },
