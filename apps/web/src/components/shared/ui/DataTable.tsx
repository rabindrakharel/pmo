...
function ColoredDropdown({ value, options, onChange, onClick }: ColoredDropdownProps) {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      {/* Dropdown menu */}
      {dropdownOpen && (
        <div className="absolute z-50 w-full mt-1 bg-dark-100 border border-dark-300 rounded-md shadow-lg max-h-60 overflow-auto">
          <div className="py-1">
            {options.map(opt => {
            ...
            <thead className="bg-gradient-to-r from-dark-100 to-dark-100/80 sticky top-0 z-30 shadow-sm">
              <tr>
                {columns.map((column, index) => (
                  <th
                    key={column.key}
                    className={`px-6 py-2.5 text-left border-r border-dark-300 ${
                            ...
                            {isEditing && column.key === '_actions' && (
                              <div className="flex items-center gap-2 mt-2">
                                ...
                                <button
                                  ...
                                  className="px-2 py-1 text-xs bg-dark-1000 text-white rounded hover:bg-dark-600 transition-colors"
                                  title="Cancel"
                                >
                                  Cancel
                                </button>
