...
echo -e "${BLUE}🛑 Stopping PMO API Server...${NC}"
...
if ! kill -0 "$API_PID" 2>/dev/null; then
    echo -e "${YELLOW}⚠️  Process $API_PID is not running. Cleaning up PID file.${NC}"
    rm -f "$PID_FILE"
    exit 0
fi
...
while kill -0 "$API_PID" 2>/dev/null && [[ $WAIT_TIME -lt $MAX_WAIT ]]; do
    sleep 1
    ((WAIT_TIME++))
