...
validate_all_ddls() {
    ...
    local ddl_files=(
        ...
        "XX_d_task_data.ddl"
        "XXI_d_artifact.ddl"
        "XXII_d_artifact_data.ddl"
        "XXIII_d_form_head.ddl"
        "XXIV_d_form_data.ddl"
        "XXV_d_wiki.ddl"
        "XXVI_d_wiki_data.ddl"
        "XXVII_d_reports.ddl"
        "XXVIII_d_report_data.ddl"
        "XXIX_entity_id_map.ddl"
        "XXX_entity_id_rbac_map.ddl"
        "XXXI_rel_emp_role.ddl"
    )
...
import_ddls() {
    ...
    execute_sql "$DB_PATH/XI_d_employee.ddl" "Employee entities with authentication"

    # Organizational hierarchy - Office first, then business units
    execute_sql "$DB_PATH/XII_d_office.ddl" "Office entity with 4-level hierarchy"
    execute_sql "$DB_PATH/XIII_d_business.ddl" "Business entity with 3-level hierarchy"

    # Supporting entities - Independent of core hierarchy
    execute_sql "$DB_PATH/XIV_d_client.ddl" "Client entities"
    execute_sql "$DB_PATH/XV_d_role.ddl" "Role entities"
    execute_sql "$DB_PATH/XVI_d_position.ddl" "Position entities"
    execute_sql "$DB_PATH/XVII_d_worksite.ddl" "Worksite entities"

    # Core project entities - Projects before tasks
    execute_sql "$DB_PATH/XVIII_d_project.ddl" "Project entities"
    execute_sql "$DB_PATH/XIX_d_task.ddl" "Task head entities"
    execute_sql "$DB_PATH/XX_d_task_data.ddl" "Task data entities"

    # Content entity tables - Documents and knowledge base
    execute_sql "$DB_PATH/XXI_d_artifact.ddl" "Artifact head entities"
    execute_sql "$DB_PATH/XXII_d_artifact_data.ddl" "Artifact data entities"
    execute_sql "$DB_PATH/XXIII_d_form_head.ddl" "Form head entities"
    execute_sql "$DB_PATH/XXIV_d_form_data.ddl" "Form data entities"
    execute_sql "$DB_PATH/XXV_d_wiki.ddl" "Wiki entities"
    execute_sql "$DB_PATH/XXVI_d_wiki_data.ddl" "Wiki data entities"
    execute_sql "$DB_PATH/XXVII_d_reports.ddl" "Report entities"
    execute_sql "$DB_PATH/XXVIII_d_report_data.ddl" "Report data entities"

    # Relationship tables - After all entities exist
    execute_sql "$DB_PATH/XXIX_entity_id_map.ddl" "Entity instance relationships"
    execute_sql "$DB_PATH/XXX_entity_id_rbac_map.ddl" "RBAC permission mapping"
    execute_sql "$DB_PATH/XXXI_rel_emp_role.ddl" "Employee-role relationships"
